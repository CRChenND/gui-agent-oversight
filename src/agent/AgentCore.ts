...
function isDynamicTool(obj: unknown): obj is DynamicTool {
  return (
    typeof obj === 'object' &&
    obj !== null &&
    'name' in obj &&
...
export class BrowserAgent {
   ...
   */
  private convertToBrowserTools(tools: unknown[]): BrowserTool[] {
    return tools.map(tool => {
      if (isDynamicTool(tool)) {
        ...
        const unknownTool = (typeof tool === "object" && tool !== null
          ? tool
          : {}) as Record<string, unknown>;
        return {
          name: typeof unknownTool.name === "string" ? unknownTool.name : "unknown_tool",
          description: typeof unknownTool.description === "string" ? unknownTool.description : "Unknown tool",
          func: async (input: string) => {
            try {
              if (typeof unknownTool.func === "function") {
                return await (unknownTool.func as (input: string) => Promise<string>)(input);
              }
...
export async function createBrowserAgent(
  ...
  // Inject global knowledge into PromptManager if configured
  try {
    const { globalKnowledgeText } = await chrome.storage.sync.get({ globalKnowledgeText: '' });
    const maybeAgent = agent as unknown as {
      promptManager?: { setGlobalKnowledgeText?: (text: string) => void };
    };
    if (globalKnowledgeText && typeof maybeAgent.promptManager?.setGlobalKnowledgeText === "function") {
      maybeAgent.promptManager.setGlobalKnowledgeText(globalKnowledgeText);
    }
  } catch (error) {
    console.warn("Failed to inject global knowledge:", error);
  }
