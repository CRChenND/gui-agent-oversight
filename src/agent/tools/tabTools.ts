...
import { getCurrentPage, setCurrentPage } from "../PageContextManager";
  ...
  new DynamicTool({
    ...
    func: async (input: string) => {
      try {
        ...
        // If we closed the currently tracked tab, choose a fresh active page from
        // the post-close list instead of using the stale pre-close snapshot.
        if (closingCurrentPage) {
          const updatedPages = page.context().pages();
          if (updatedPages.length > 0) {
            const newActiveIdx = Math.min(Math.max(0, idx - 1), updatedPages.length - 1);
            const newActivePage = updatedPages[newActiveIdx];
            setCurrentPage(newActivePage);

            try {
              const newActiveTabId = await getCurrentTabId(newActivePage);
              const newActiveTitle = await newActivePage.title();
              if (newActiveTabId !== undefined) {
                ...
                console.log(
                  `Tab ${idx} closed, switched to tab ${newActiveIdx} with title "${newActiveTitle}"`
                );
              }
            } catch (titleError) {
