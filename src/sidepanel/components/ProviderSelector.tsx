...
export function ProviderSelector({ isProcessing }: ProviderSelectorProps) {
  ...
  const loadOptions = async () => {
    ...
    for (const provider of providers) {
      ...
      providerOptions.push({
        ...
        models: models.length > 0 ? models : defaultModelsFor(provider, config.apiModelId || ''),
      });
    }
  ...
  const defaultModelsFor = (provider: string, currentModelId: string): {id: string, name: string}[] => {
    if (provider === 'openrouter') {
      const id = currentModelId || 'openai/gpt-4o';
      return [{ id, name: id }];
    }
    return [];
  };
