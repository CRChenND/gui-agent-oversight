...
export interface ProviderConfig {
  provider: 'anthropic' | 'openai' | 'gemini' | 'ollama' | 'openai-compatible' | 'openrouter';
  apiKey: string;
  apiModelId?: string;
  baseUrl?: string;
...
export class ConfigManager {
    ...
    switch (result.provider) {
      ...
      case 'openrouter':
        return {
          ...
          apiModelId: result.openrouterModelId,
          baseUrl: result.openrouterBaseUrl || 'https://openrouter.ai/api/v1',
        };
      default:
  ...
  async getModelsForProvider(provider: string): Promise<{id: string, name: string}[]> {
    switch (provider) {
      ...
      case 'openrouter': {
        const result = await chrome.storage.sync.get({ openrouterModelId: '' });
        const id = result.openrouterModelId || '';
  ...
  async updateProviderAndModel(provider: string, modelId: string): Promise<void> {
    ...
    switch (provider) {
      ...
      case 'ollama':
        ...
        break;
      case 'openrouter':
        await chrome.storage.sync.set({ openrouterModelId: modelId });
        break;
    }
  }
}
